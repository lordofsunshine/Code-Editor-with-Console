...
- **Совместная работа**: мгновенные изменения, позиции курсоров, присутствие участников.
- **Проекты и файлы**: создание, загрузка, удаление, структурирование ресурсов.
...
- **Современный UX**: тёмная/светлая темы, статус‑бар, вкладки, хоткеи.
...
- **Уведомления о кике**: События WebSocket при удалении пользователей из проектов с автоматическим редиректом и уведомлением
- **Настройки редактора**: Теперь можно настроить редактор, как Вы хотите!
- **Усиленная безопасность**: Полная валидация входных данных с проверками `isNaN()`, защита от SQL-инъекций, XSS, ролевой контроль доступа
- **Оптимизация БД**: Автоматический VACUUM (еженедельно), PRAGMA optimize (ежедневно), инкрементальная очистка для повышения производительности
- **SEO-готовность**: Полные мета-теги (Open Graph, Twitter Cards, описания), favicon.svg, robots.txt, sitemap.xml для всех доменов (code-editor.run, code-editor.app, code-editor.lol)
- **Анимации приветствия**: Плавные анимации звёзд и комет на стартовом экране с оптимизированной производительностью и чистой архитектурой
- **Организация кода**: Вспомогательные функции вынесены в отдельный файл utils.js для лучшей поддерживаемости
- **Контроль доступа**: Усиленные проверки прав на всех API-эндпоинтах с корректной обработкой ошибок
