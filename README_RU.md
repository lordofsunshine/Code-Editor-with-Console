...
1. **Более безопасный предварительный просмотр в редакторе**
 Удален параметр "разрешить тот же источник" из изолированного iframe предварительного просмотра в "public/editor.html`. Это делает область предварительного просмотра более изолированной и безопасной.

2. **Более безопасный поиск контента**
 Поиск по содержимому теперь имеет ограничения по размеру файла, общему бюджету поиска, количеству файлов и количеству результатов. Это помогает защитить сервис от интенсивного поиска или злоупотреблений.

3. Предварительный просмотр вместо полного содержимого**
 Результаты поиска теперь возвращают только небольшой фрагмент предварительного просмотра вместо полного содержимого файла. Это делает поиск быстрее, проще и безопаснее.

4. **Блокировка при переносе файлов**
 Добавлена блокировка для каждого файла при перемещении сохраненного содержимого базы данных на диск. Это помогает предотвратить конфликты при одновременной обработке одного и того же файла.

5. ** Улучшена защита HTML-кода**
 Обновлено экранирование HTML в клиентских скриптах на страницах предварительного просмотра, дерева файлов, чата, поиска, редактора, авторизации, совместной работы, предупреждений, пользовательского меню и "Что нового".

6. ** Более строгие правила экранирования текста.**
 Текст, обращенный к пользователю, теперь экранируется `&`, `<`, `>`, `"`, и ``. Это снижает риск отображения небезопасного HTML-кода в интерфейсе.

Последние обновления и улучшения:

- **Поиск по содержимому**: Поиск по тексту работает с файловым хранилищем и корректно обрабатывает старые базы
- **Наследуемое шифрование**: Отсутствующие ключи проектов генерируются автоматически, мигрированные данные очищаются
- **Права в реальном времени**: Просмотрщики не могут отправлять события редактирования/создания/удаления, улучшена валидация данных
